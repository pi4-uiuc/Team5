...
print(euclidean(gt_file[,2],gt_file[,3]))
...
for (i in 2:305){
  euclid_distances[i-1] <-euclidean(gt_file[,i],'0|0')
  i
}
...
plot(cl_tree,main='Hamming Distance Clusters', labels = as.vector(key_names))
dev.off()
...
cl_tree <- hclust(as.dist(cd_matrix))
...
```{r}
png("Cityblock_clusters.png",width=1800,height = 400)
plot(cl_tree,main='Cityblock Distance Clusters')
dev.off()
```

And finally, run a version for Euclidian